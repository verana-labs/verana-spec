...
| `verana.vsa.flow.validateFlow` | [`validateFlow`](../vs-agent/spec.md#vsa-adm-vt-fl-validate-validateflow) | Mark the applicant's documentation as validated. Starts issuance for a Credential Direct Issuance flow. For an Onboarding Process flow: validates the claims of a `HOLDER` flow, records the agreed fees, and submits `SetParticipantOPtoValidated` itself when the agent holds a VS operator authorization that covers it, or leaves the transaction to an operator (see vs-agent [VSA-ADM-VT-FL-VALIDATE](../vs-agent/spec.md#vsa-adm-vt-fl-validate-validateflow)). |
