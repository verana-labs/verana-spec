...
| `verana.vsa.flow.sendValidating` | [`sendValidating`](../vs-agent/spec.md#vsa-adm-vt-fl-validating-sendvalidating) | Tell the applicant that the out-of-band step is complete and the validator is reviewing; moves the flow from `OOB_PENDING` to `VALIDATING`. |
...
| `verana_mcp_ws_connected` | gauge | `endpoint` | `1` if the named WS (`endpoint` ∈ `rpc`, `indexer`, `graph`) is connected, `0` otherwise. |
