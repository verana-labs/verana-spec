...
| Variable | Required | Description |
|---|---|---|
| `PUBLIC_API_BASE_URL` | REQUIRED | Public `https://` base URL at which a peer reaches the public endpoints of the agent. The agent derives its DID from this value and composes each protocol URL from it verbatim. A base path is allowed. The agent MUST reject a URL that carries a username or a password. The deployment MUST serve this URL with a certificate from a publicly trusted certification authority: a wallet fails closed on a TLS error, before any protocol step, and reports nothing that an operator can act on. See [[VSA-VTI-BOOT-DID] DID Creation](#vsa-vti-boot-did-did-creation). |
...
| `ADMIN_API_TRUSTED_NETWORKS` | OPTIONAL | Comma-separated list of CIDR blocks. The agent classifies a request as trusted-network when the peer address of its TCP connection matches one block, and serves that request without authentication, in both modes. Default: `127.0.0.0/8,::1/128`, which admits a caller on the loopback interface only. A deployment whose backend runs in another container, or in another pod, MUST add the network of that caller or use `corporation` mode: the peer address of such a call is the address of the caller, never a loopback address. The operator MUST keep the source address of each public reverse proxy or ingress outside these blocks. See [Trusted networks](#vsa-adm-access-net-trusted-networks). |
...
[VSA-PUB-OID-1] **Well-known location.** The paths of the table are the form that a deployment without a base path serves. When `PUBLIC_API_BASE_URL` carries a base path, each of the three metadata documents moves: OpenID4VCI, [RFC 8414](https://www.rfc-editor.org/rfc/rfc8414) and SD-JWT VC all insert the well-known segment between the host and the path of the identifier, so the agent MUST serve `/.well-known/openid-credential-issuer/{base path}`, `/.well-known/oauth-authorization-server/{base path}` and `/.well-known/jwt-vc-issuer/{base path}`. A wallet that concatenates instead of inserting is common, so the agent SHOULD answer the concatenated form as well, with the same document.

[VSA-PUB-OID-2] **Signed and plain credential issuer metadata.** The agent MUST serve its credential issuer metadata as a signed JWT, with the certificate of the issuer capability in `x5c`, to a client that accepts `application/jwt` alone, and as a JSON document to a client that accepts `application/json`. When a client accepts both, the agent MUST serve the JSON document: the signed form obliges the client to resolve the signer, and a wallet that cannot resolve the DID method of the agent rejects a document that it would otherwise read.

[VSA-PUB-OID-3] **Advertised proof types.** The credential issuer metadata advertises the proof types that the agent accepts: `jwt` always, and `attestation` only when the operator configures `issuer.keyAttestationCertificates`. The agent MUST NOT advertise `key_attestations_required` on a proof type that it cannot verify an attestation for, and MUST NOT refuse a proof that carries an attestation it did not ask for. Wallet families disagree over this one flag while sending an identical `Accept` header: one refuses to issue when it is present, another requires it. An agent MAY therefore vary this flag per client, and MUST NOT vary any other part of the document.

[VSA-PUB-OID-4] **Authorization-server metadata.** The agent MUST advertise the DPoP signing algorithms that it accepts, as `dpop_signing_alg_values_supported`: a wallet that reads that parameter without testing whether it is present cannot complete a token request against a document that omits it. When the operator configures `issuer.walletAttestationCertificates`, the metadata MUST also advertise the attestation-based client authentication method and the algorithms that the agent accepts for it.
...
The scope mirrors the [DIDComm Scope](#didcomm-scope): a credential offer and a presentation request produce a URL that the caller renders as a QR code or sends as a link, and each one starts an exchange that the caller then reads by identifier. Those URLs carry the `openid-credential-offer://` and `openid4vp://` schemes, so a wallet that registers neither scheme is reachable by a scanned QR code only. Two differences follow from the protocol:
...
- `url` — the credential offer URI, with the scheme `openid-credential-offer://`, ready to render as a QR code or to send as a link.
...
- `requestSigner` (OPTIONAL, default `x5c`) — `x5c` or `did`. `x5c` names the signing certificate of the verifier capability, for a wallet that cannot resolve a DID: the agent MUST use the `x509_san_dns` client identifier when that certificate chains to a publicly trusted root and carries a DNS SAN equal to the host of `PUBLIC_API_BASE_URL`, and `x509_hash` otherwise. A development certificate is self-signed, so it never satisfies the first case, and a wallet that validates the chain behind an `x509_san_dns` identifier refuses such a request: that is a property of development signing, not of the wallet. `did` names the DID of the agent as the client identifier of the authorization request, and the signing key MUST be published under `authentication`.
...
- `url` — the authorization request URI, with the scheme `openid4vp://`, ready to render as a QR code or to send as a link.
...
| `VSA-PUB-OID-1` | **Well-known location.** The paths of the table are the form that a deployment without a base path serves. When `PUBLIC_API_BASE_URL` carries a base path… |  |
| `VSA-PUB-OID-2` | **Signed and plain credential issuer metadata.** The agent MUST serve its credential issuer metadata as a signed JWT, with the certificate of the issuer capability in `x5c`… |  |
| `VSA-PUB-OID-3` | **Advertised proof types.** The credential issuer metadata advertises the proof types that the agent accepts: `jwt` always, and `attestation` only when the operator configures… |  |
| `VSA-PUB-OID-4` | **Authorization-server metadata.** The agent MUST advertise the DPoP signing algorithms that it accepts, as `dpop_signing_alg_values_supported`… |  |
...
- **v4-draft10, third revision (2026-09-09)** — What a wallet needs from the OpenID4VC endpoints, from the behaviour that the reference deployment and the wallet matrix already rely on. The three metadata documents move to the well-known location that OpenID4VCI, [RFC 8414](https://www.rfc-editor.org/rfc/rfc8414) and SD-JWT VC define for an identifier with a base path; credential issuer metadata is served signed or plain by content negotiation; the advertised proof types, the `key_attestations_required` flag and the DPoP algorithms are specified ([[VSA-PUB-OID]](#vsa-pub-oid-openid4vc-public-protocol-endpoints)). An `x5c` presentation request names an `x509_san_dns` client identifier when its certificate is publicly trusted and an `x509_hash` one otherwise ([[VSA-ADM-OID-PR-CREATE]](#vsa-adm-oid-pr-create-createpresentationrequest)), the two URLs of the scope carry their protocol schemes, `PUBLIC_API_BASE_URL` needs a publicly trusted certificate, and `ADMIN_API_TRUSTED_NETWORKS` says what a backend in another pod has to add.
