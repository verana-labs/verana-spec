...
- [VFE-PAGE-AGENTS-1] **Derivation.** An **agent** is identified by a DID and is derived, not queried directly: from the acting Corporation's active `VSOperatorAuthorization` entries ([`IDX-DE-QRY-2`](../verana-indexer/spec.md#idx-de-qry-2-list-vs-operator-authorizations) with `corporation_id=<acting>&only_active=true`), each `ParticipantAuthorizationRecord.participant_id` is resolved to its Participant ([`IDX-PP-QRY-2`](../verana-indexer/spec.md#idx-pp-qry-2-list-participants)) and agents are **grouped by `Participant.did`**. An agent MUST appear as soon as at least one active record resolves to its DID, and MUST disappear when the last such record is removed or expires. Per VPR [`MOD-DE-MSG-5-2`](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-de-msg-5-2-grant-vs-operator-authorization-basic-checks), a `vs_operator` account belongs to at most one Corporation, so this derivation is complete. The chain does not enforce a 1:1 account↔DID binding (that is a VS Agent runtime property), so the page MUST tolerate one DID served by several `vs_operator` accounts (a single card listing every account) and one account referenced from several DIDs (shown on each DID's card).
