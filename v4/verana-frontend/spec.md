...
- [VFE-PAGE-PENDING-3] Where applicant evidence review happens outside the frontend — agent-side, or in the validator's Ecosystem Onboarding Service — the frontend MUST NOT pretend to hold it: the task view surfaces the on-chain facts and the on-chain actions, and deep-links to that tooling per [VFE-PAGE-PENDING-4].
- [VFE-PAGE-PENDING-4] **Portal deep link.** For each pending task, the frontend MUST resolve the DID of the task's validator Participant (the entry referenced by `validator_participant_id`; one [`IDX-VT-QRY-1`](../verana-indexer/spec.md#idx-vt-qry-1-resolve) call per distinct validator DID selecting `services`, cached per [VFE-DATA-RESOLVE-2]) and, when its DID Document declares a service entry of type `EcosystemOnboardingService` (an [Ecosystem Onboarding Service](../onboarding-service/spec.md#obs-boot-svc-service-entry-registration) deployed by the validator Corporation), MUST show a **Review in portal** action linking to `{serviceEndpoint}/cases/{participant_id}`, opened in a new window. The link is an affordance, not a replacement: the on-chain actions of [VFE-PAGE-PT-4] stay available, and a validator that reviews in the portal typically signs `SetParticipantOPtoValidated` there.
