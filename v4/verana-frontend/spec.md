...
- [VFE-TX-COSTS-2] Amounts expressed in a Credential Schema's pricing asset (`pricing_asset_type`, `pricing_asset` — TU, COIN, or FIAT) MUST be converted for display and for balance checks via [`IDX-XR-QRY-3 Get Price`](../verana-indexer/spec.md#idx-xr-qry-3-get-price). An expired or missing exchange rate MUST surface as an explicit "cannot estimate" state, not as zero.
- [VFE-TX-COSTS-3] Starting or renewing an Onboarding Process MUST preview: the validator Participant's `validation_fees` (converted per [VFE-TX-COSTS-2]) plus the applicant-side trust deposit (`validation fees × trust_deposit_rate`, always in native denom), per [`MOD-PP-MSG-1`](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-1-start-participant-op) fee semantics.
...
- [VFE-DATA-WS-1] While a wallet is connected and an acting Corporation is selected, the frontend MUST hold one WebSocket subscription to `/v4/indexer/subscribe` scoped with `corporationId=<acting Corporation id>`, re-subscribing on switch ([VFE-CORP-SEL-2]). In guest mode the socket MAY be held unsubscribed or closed.
...
- [VFE-DATA-WS-3] Received events MUST drive targeted refreshes: Participant events refresh participant views and the pending-tasks badge; Delegation and `payload.module = "group"` events trigger [VFE-CORP-DISC-4] and the proposals badge [VFE-CORP-PROP-2]; Corporation/Ecosystem/Schema events refresh the corresponding views when open.
...
- [VFE-PAGE-CORP-3] **Trust Deposit**: from [`IDX-TD-QRY-1`](../verana-indexer/spec.md#idx-td-qry-1-get-trust-deposit-by-corporation): `deposit`, `claimable` yield, slash history; **Reclaim yield** (`MsgReclaimTrustDepositYield`, shown only when `claimable > 0`) and **Repay slashed deposit** (`MsgRepaySlashedTrustDeposit`, shown only when an unrepaid slash exists, with the exact required amount). The UI MUST reflect that v4 trust deposits are non-withdrawable — no withdraw affordance exists.
...
- [VFE-PAGE-CS-1] From [`IDX-CS-QRY-1`](../verana-indexer/spec.md#idx-cs-qry-1-get-credential-schema): header (title, description, archived badge, onboarding-mode badges for issuer/verifier/holder), immutable facts (id, ecosystem, pricing asset, digest algorithm), counters.
...
| Fee & cost previews | `IDX-TD-QRY-2`, `IDX-ES-QRY-3`, `IDX-CS-QRY-4`, `IDX-CO-QRY-3`, `IDX-XR-QRY-3`, `IDX-DE-QRY-5` |
