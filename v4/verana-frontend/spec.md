...
### [VFE-GEN-DEPLOY] Deployment and Configuration
...
|---|---|---|
| `NEXT_PUBLIC_BASE_URL` | REQUIRED | Public origin the deployment is served from (e.g. `https://app.devnet.verana.network`). |
...
| `NEXT_PUBLIC_VERANA_REST_ENDPOINT` | REQUIRED | Chain LCD (REST) endpoint URL, passed to the wallet kit's chain registration ([VFE-WALLET-1]); the frontend itself reads chain data through the RPC only ([VFE-DATA-SRC-1]). |
| `NEXT_PUBLIC_VERANA_INDEXER_BASE_URL` | REQUIRED | Indexer base URL (e.g. `https://idx.testnet.verana.network`). All REST paths are `{base}/v4/...` per the [Indexer v4 Specification](../verana-indexer/spec.md); the WebSocket endpoint is derived as `{base}/v4/indexer/subscribe` with the scheme switched to `ws(s)`, per [VFE-GEN-DEPLOY-4]. |
| `NEXT_PUBLIC_VERANA_EXPLORER_URL` | REQUIRED | Block explorer base URL, used for transaction and account links. |
| `NEXT_PUBLIC_VERANA_VISUALIZER_URL` | REQUIRED | Read-only explorer/visualizer base URL, used for entity deep links. |
| `NEXT_PUBLIC_VERANA_TOPUP_VS` | REQUIRED | DID of the Verifiable Service used by the Get VNA flow ([VFE-PAGE-ACCT]). |
...
- [VFE-GEN-LAYOUT-2] The header MUST contain: app logo and name; the **acting Corporation selector** ([VFE-CORP-SEL]) when a wallet is connected; settings access; light/dark theme toggle; and the wallet/account zone (connect button, or connected-account chip with address copy, explorer link, and disconnect). On small screens the selector MUST remain reachable from the collapsed navigation menu.
...
- [VFE-GEN-SRV-2] The frontend MUST provide a server-side **verified fetch** route: given an `https` URL and an expected `digest_sri`, it fetches the resource under the same restrictions as [VFE-GEN-SRV-1], verifies the bytes against the digest, and returns them with their media type, or an error on mismatch — never unverified bytes.
- [VFE-GEN-SRV-3] A **governance framework document viewer** MUST render EGF and CGF documents only from bytes verified against the on-chain `digest_sri`: fetched client-side and hashed with WebCrypto, or obtained through [VFE-GEN-SRV-2] when CORS prevents the client fetch. It MUST present a truthful verification state (verified, digest mismatch, could not verify), MUST NOT claim verification it did not perform, MUST offer every document of the displayed version with a language selector, and MUST show the digest. Markdown and PDF render inline; HTML renders in a sandboxed frame without scripts; other types download only.
- [VFE-GEN-SRV-4] **Digest computation and verification.** A governance framework document's `digest_sri` is the Subresource Integrity form of the SHA-384 hash of the exact bytes served at its URL, `sha384-` followed by the base64 hash. Verification MUST hash in the client with WebCrypto and compare against the on-chain value. The client fetch is attempted first; [VFE-GEN-SRV-2] is used only when it fails, and the bytes that route returns MUST be re-hashed in the client rather than trusted on the route's word. An on-chain value that is not a well-formed `sha384-` digest, and any failure to obtain bytes, MUST yield **could not verify**, never a mismatch and never a verification; only differing bytes, including the mismatch [VFE-GEN-SRV-2] reports, yield **digest mismatch**.
- [VFE-GEN-SRV-5] **Route hardening.** The routes of [VFE-GEN-SRV-1] and [VFE-GEN-SRV-2] MUST share one outbound fetch path that accepts `http` and `https` only; refuses a host that is, or resolves to, a loopback, link-local, private, reserved or otherwise non-public address, and refuses the `localhost`, `local`, `internal` and `home.arpa` name suffixes; follows redirects itself, re-applying those checks to every hop, with a bounded hop count; bounds the response size and the total time; and is rate limited per client ([VFE-SEC-4]). [VFE-GEN-SRV-2] MUST answer a digest mismatch with a status distinct from a fetch failure, so that the caller can tell the two apart, and MUST serve verified bytes non-renderably: as an attachment, with content-type sniffing disabled, under a sandboxing `Content-Security-Policy`, and uncached.
- [VFE-GEN-SRV-6] **Failed verification.** The viewer of [VFE-GEN-SRV-3] MUST NOT render or offer for download a document it did not verify: on **digest mismatch** and on **could not verify** it shows the state, the on-chain digest and the reason, and nothing of the content. A link to the document's own URL MAY remain, since it opens outside the frontend. A **digest mismatch** MUST also block the governance framework acceptance of [VFE-PAGE-DISCOVER-2] and [VFE-OBS-4] and clear an acceptance already given; **could not verify** warns next to the acceptance without blocking it.
...
- [VFE-WALLET-4] Session state (acting Corporation, list filters) MUST be persisted client-side with a lifetime of `NEXT_PUBLIC_SESSION_LIFETIME_SECONDS` and MUST be invalidated on wallet disconnect or account switch. Disconnect means the explicit disconnect action or an account switch: a page reload, a wallet lock or a transient loss of the address keep the persisted acting Corporation, which [VFE-WALLET-2] restores subject to the re-validation of [VFE-CORP-SEL-3]. Display preferences (locale, theme) are not session state: they are account-independent and persist separately per [VFE-PAGE-SET-2].
...
- [VFE-CORP-DISC-5] **Partial discovery.** A pass is **partial** when one of the two sources of [VFE-CORP-DISC-1] failed, or when a discovered Corporation could not be resolved per [VFE-CORP-DISC-2]. A failed source contributes nothing; it MUST NOT be read as an empty result. The frontend MUST merge a partial pass over the memberships it already knows for that account, the freshly returned entry winning and a previously known Corporation the pass did not return being kept, and MUST NOT conclude from a partial pass that a membership was lost: no known membership may be dropped, the persisted acting-Corporation choice MUST NOT be forgotten, and no Corporation may be auto-selected ([VFE-CORP-SEL-2]). Only a pass where every source answered redefines the discovered set. A partial pass MUST be visible in the selector, with the failure reason available to the user.

### [VFE-CORP-SEL] Acting Corporation Selection

- [VFE-CORP-SEL-1] Whenever a wallet is connected, the header MUST show the acting Corporation selector: the current acting Corporation (or a "no corporation" state), and a dropdown listing every discovered Corporation with its membership-kind badges (*Operator*, *Member*) and, for members, their voting weight. Each Corporation is displayed by its resolved organization or service name per [VFE-TRUST-CLAIMS] with the DID as subtitle and the country flag when the organization credential carries a `countryCode`; without a resolvable name the DID alone is shown, in full and with no placeholder flag: middle truncation MUST NOT be applied to it, since truncated DIDs made two `did:web` Corporations indistinguishable. The dropdown MUST always end with a **Create new Corporation** entry opening the wizard of [VFE-CORP-CREATE] — an account can create additional Corporations regardless of how many it already acts for.
- [VFE-CORP-SEL-2] Exactly one Corporation is acting at a time. Switching MUST re-scope every corporation-bound surface (pages of [VFE-PAGE], event subscription of [VFE-DATA-WS], capability gates of [VFE-CORP-CAPS]) without requiring a reload. If the set has exactly one entry, it is auto-selected and that auto-selection is persisted like a manual choice; if several, the frontend MUST prompt for selection on first connect with a blocking chooser dialog reusing the selector's row display, and persist the choice per [VFE-WALLET-4]. The chooser is a modal dialog and MUST NOT be dismissable (Escape, backdrop) without a choice. Actions read the acting Corporation from this selection only: while discovery is loading, corporation-bound action buttons MUST be disabled, and when the account has memberships but no selection, triggering an action MUST open the chooser instead of broadcasting.
- [VFE-CORP-SEL-3] On session restore, the persisted acting Corporation MUST be re-validated against a fresh and full discovery pass (both sources of [VFE-CORP-DISC-1] answered); if the account can no longer act for it, the selection MUST be cleared and the user re-prompted. When the pass was only partial because one source failed, the selection MUST be kept, not cleared, and re-validated on the next pass.
...
- [VFE-CORP-SEL-5] The selector MUST show, next to each **non-acting** discovered Corporation, attention indicators when that Corporation has work waiting — pending validator tasks and proposals awaiting the vote MUST stay visually distinct (separate badges with accessible labels): pending validator tasks (count from [`IDX-PP-QRY-5`](../verana-indexer/spec.md#idx-pp-qry-5-pending-flat) with that `corporation_id`) and/or proposals awaiting the connected account's vote ([`IDX-GR-QRY-3`](../verana-indexer/spec.md#idx-gr-qry-3-list-proposals) with `pending_voter=<account>` and that `corporation_id`). Counts are initialized at discovery and kept fresh event-driven per [VFE-DATA-WS-1] / [VFE-DATA-WS-3].
- [VFE-CORP-SEL-6] The pending validator tasks count of [VFE-CORP-SEL-5], and the Pending Tasks badge of [VFE-PAGE-NAV-1], MUST count validator-side tasks only: entries whose validator Participant is owned by that Corporation, never the Corporation's own applications. [`IDX-PP-QRY-5`](../verana-indexer/spec.md#idx-pp-qry-5-pending-flat) is specified as validator-scoped but currently also returns applicant-side rows under the applicant's `corporation_id`; whether the route gains an explicit validator-only filter or the implementation is aligned to its text is an open indexer question.
- [VFE-CORP-SEL-7] The blocking chooser of [VFE-CORP-SEL-2] MUST open whenever discovery has settled, no Corporation is acting, and at least one membership is known. This includes the case of a single known membership, which the auto-selection of [VFE-CORP-SEL-2] otherwise makes unreachable: after a partial pass ([VFE-CORP-DISC-5]) nothing is auto-selected, so a sole membership is offered in the chooser rather than assumed.
...
- [VFE-CORP-CAPS-3] When (a) holds but (b) does not and the account is a group member, the frontend MUST offer the **proposal fallback** ([VFE-TX-FALLBACK]) instead of hiding the action: "submit as group proposal". The fallback MUST be surfaced inside the confirmation step of [VFE-TX-SIM]: signing mode **Governance Proposal** with an explainer naming the Corporation, a **Submit proposal** confirm label, and the proposal title defaulting to the action label. The frontend MUST NOT dead-end the member with an authorization error.
...
- [VFE-CORP-CAPS-5] **Signing-mode icon.** Every action affordance that leads to a transaction on a Corporation resource MUST carry a discreet icon telling the user, before they click, how the action will execute for the connected account: **Operator** mode — direct execution, the account holds an `OperatorAuthorization` covering the Msg type (RECOMMENDED Font Awesome icon: `fa-bolt`) — or **Governance Proposal** mode — no covering authorization, the action opens the proposal flow of [VFE-TX-FALLBACK] (RECOMMENDED Font Awesome icon: `fa-gavel`). The icon MUST have an accessible label/tooltip naming the mode, and the displayed mode MUST be recomputed whenever the capability set changes ([VFE-CORP-DISC-4]). Group Msgs themselves (vote, execute, withdraw, submit proposal) are always member-signed and carry no mode icon.

### [VFE-CORP-CREATE] Corporation Creation

- [VFE-CORP-CREATE-1] Any connected account MUST be able to create a Corporation — no pre-existing membership or authorization is required (per VPR, any account may sign `MsgCreateCorporation`). The wizard MUST be reachable from: (a) the **Create new Corporation** entry of the acting-Corporation selector ([VFE-CORP-SEL-1]); (b) a prominent empty-state call-to-action when discovery ([VFE-CORP-DISC]) yields no Corporation for the connected account, shown on the Dashboard and doubled by the **Create new Corporation** entry of the header selector, since [VFE-PAGE-NAV-2] hides the Corporation entry in that state; and (c) the Corporation page ([VFE-PAGE-CORP]): as the empty-state call-to-action when no Corporation is acting, and as a header-level **New Corporation** action when one is. It drives [`MOD-CO-MSG-1 Create Corporation`](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-co-msg-1-create-corporation), collecting: **members** (addresses + weights, at least one), **decision policy** (threshold or percentage, voting period, min execution period), **Corporation DID**, **primary language** (BCP 47), and the **CGF v1 document** (URL + `digest_sri` computed via [VFE-GEN-SRV-1]). The wizard is staged: **identity** (DID, language, CGF document), **members & policy**, **review**, then the guided first operator grant of [VFE-CORP-CREATE-3]. The review step MAY collect an initial funding amount for the new `policy_address`, bundled as a bank send with the follow-up grant proposal. The wizard MAY fix the decision policy's minimum execution period to 0 so that the guided self-grant proposal of [VFE-CORP-CREATE-3], submitted with `EXEC_TRY`, executes immediately; collecting the period from the user breaks the guided grant.
...
- [VFE-CORP-CREATE-3] On success the frontend MUST re-run discovery; if the creator included themselves as a member, it SHOULD offer the guided next step "propose an OperatorAuthorization for your account" (pre-filled proposal per [VFE-CORP-PROP-4]) as the wizard's final, skippable step.
...
- [VFE-CORP-PROP-1] The Corporation page MUST include a **Proposals** view listing the acting Corporation's proposals from [`IDX-GR-QRY-3 List Proposals`](../verana-indexer/spec.md#idx-gr-qry-3-list-proposals) (`corporation_id=<acting>`), filterable by `status`, showing per row: id, submit time, proposers, status, `voting_period_end`, executor result, and the running `tally` against the group's decision policy. When the connected account is a group member, the view MUST show a prominent **New proposal** action opening the composer of [VFE-CORP-PROP-4]. The view SHOULD follow this reference presentation: expandable proposal cards showing id, status, proposers, submit time or the voting-period countdown, and the running weighted tally as a bar against the group's threshold, under a status filter (All, Open, Accepted, Rejected, Withdrawn) with counts; the expanded card shows the decoded messages, the votes with their options, and the vote (yes, no, abstain, no-with-veto), execute and withdraw actions of [VFE-CORP-PROP-3].
- [VFE-CORP-PROP-2] The navigation MUST show a **pending-proposals badge** on the **Corporation** entry ([VFE-PAGE-NAV-1]): the count from `IDX-GR-QRY-3` with `pending_voter=<connected account>`, updated in real time via the `x/group` events of the corporation-scoped subscription ([VFE-DATA-WS]).
...
- [VFE-TX-COSTS-5] **Trust units.** The module deposits of [VFE-TX-COSTS-1] are expressed in trust units, not in the native denom: the preview MUST multiply `ecosystem_trust_deposit` or `credential_schema_trust_deposit` by the `trust_unit_price` of [`IDX-ES-QRY-3`](../verana-indexer/spec.md#idx-es-qry-3-get-ecosystem-params) before displaying an amount. When a parameter the line depends on could not be read, the line MUST be omitted; a zero or a hardcoded rate MUST NOT stand in for it.
- [VFE-TX-COSTS-6] **Previewed flows.** Beyond the network fee, the confirmation of [VFE-TX-SIM-1] MUST show: for `MsgCreateEcosystem` and `MsgCreateCredentialSchema`, the module trust deposit of [VFE-TX-COSTS-5]; for `MsgStartParticipantOP` and `MsgRenewParticipantOP`, the two lines of [VFE-TX-COSTS-3]; for `MsgRepaySlashedTrustDeposit` and `MsgRepayParticipantSlashedTrustDeposit`, the exact amount owed. Each of these lines is a **debit** of the signer's balance, and the frontend MUST keep the debit total separable from the display strings so that [VFE-TX-COSTS-7] can use it.
- [VFE-TX-COSTS-7] **Balance check.** When the connected account is the fee payer ([VFE-TX-FEEGRANT]), the confirmation MUST compare its spendable balance against the simulated network fee **plus** the debit total of [VFE-TX-COSTS-6], and distinguish two states: a **shortfall**, where the balance does not cover that total, and the **low balance** of [VFE-WALLET-5], where it does but falls under `NEXT_PUBLIC_LOW_BALANCE_WARN_UVNA`. Both MUST name the balance and the required total. No balance warning is shown when the Corporation pays through a fee grant.
- [VFE-TX-COSTS-8] **Fee distribution.** The preview of [VFE-TX-COSTS-4] MUST apply the paying Participant's `issuance_fee_discount` or `verification_fee_discount` to each beneficiary's `issuance_fees` or `verification_fees`, show one line per beneficiary with its role and Participant id, then the total and the trust deposit that total attracts (`total × trust_deposit_rate`). It is offered for ISSUER and VERIFIER Participants only, the roles [`IDX-PP-QRY-4`](../verana-indexer/spec.md#idx-pp-qry-4-find-beneficiaries) takes as its arity, and MUST be omitted, not shown empty, when the beneficiary query returns nothing.
...
- [VFE-DATA-WS-5] Provider order: the subscription set of [VFE-DATA-WS-1] is a function of discovery, so the event layer sits below the corporation context and reads the discovered set from it; events flowing back re-run discovery per [VFE-CORP-DISC-4] and never define the discovered set themselves.
...
- [VFE-PAGE-NAV-1] Primary navigation: **Dashboard**, **Account**, **Corporation**, **Ecosystems**, **Agents**, **Discover & Join**, **Pending Tasks** (with a badge: pending validator tasks only, [VFE-PAGE-PENDING-2]), **Settings**. The Corporation entry carries the badge of proposals awaiting the connected account's vote ([VFE-CORP-PROP-2]). The two counts MUST NOT be combined: the Pending Tasks page lists no proposals, so a combined count could not be reconciled with the page it leads to.
- [VFE-PAGE-NAV-2] Guest mode exposes Dashboard, Discover (read-only), and Settings; Account requires a connected wallet; Corporation, Ecosystems, Agents, and Pending Tasks require an acting Corporation.
...
- [VFE-PAGE-DASH-5] **Health states.** Each component of [VFE-PAGE-DASH-2] carries one of three states, and a component whose health query failed MUST render as **unknown**, never as healthy and never as stalled. The indexer state comes from [`IDX-INDEXER-QRY-2 Get Indexer Status`](../verana-indexer/spec.md#idx-indexer-qry-2-get-indexer-status): **running** while `is_crawling` is true, **stalled** otherwise, and when stalled the card MUST surface `stopped_reason`, falling back to `stopped_at`. The chain state comes from the RPC status query ([VFE-DATA-SRC-2]): **catching up** when the node reports it, **synced** otherwise, alongside the node version, the chain's latest height and its network id. Next to the last indexed height of [VFE-PAGE-DASH-3], the indexer card MUST show the time of that block. Each state MUST be readable without colour alone.
...
- [VFE-PAGE-ACCT-4] **Address affordances.** The address MUST be rendered in full, never middle-truncated. Copy MUST confirm visibly and transiently that the address reached the clipboard, and MUST NOT report success when the clipboard write failed. The QR affordance opens a dialog that repeats the address as selectable text next to the code. The explorer link MUST target the account view of `NEXT_PUBLIC_VERANA_EXPLORER_URL` and MUST be omitted, not disabled, when the deployment configures no explorer.
- [VFE-PAGE-ACCT-5] **Membership rows.** The list of [VFE-PAGE-ACCT-2] MUST reuse the selector's row display ([VFE-CORP-SEL-1]): resolved organization or service name with the country flag and the DID as subtitle, the DID alone and in full when no name resolves, the *Operator* and *Member* badges with the member's voting weight, and the attention badges of [VFE-CORP-SEL-5] on the rows that are not acting. The acting Corporation MUST be marked as such. Activating a row MUST set the acting Corporation and navigate to the Corporation page in one step, never navigate without switching.
...
- [VFE-PAGE-CORP-5] **Operator Authorizations**: active `OperatorAuthorization` entries ([`IDX-DE-QRY-1`](../verana-indexer/spec.md#idx-de-qry-1-list-operator-authorizations) by `corporation_id`) with msg types (each row SHOULD list its granted Msg types grouped by module, expandable), limits, expiration; associated fee grants from [`IDX-DE-QRY-5`](../verana-indexer/spec.md#idx-de-qry-5-list-fee-grants). The section MUST offer **Grant operator** and, per row, **Revoke** actions: executed directly (`GrantOperatorAuthorization` / `RevokeOperatorAuthorization`) when the connected account holds the corresponding operator capability, and otherwise — the common bootstrap case — submitted as a group proposal via the composer of [VFE-CORP-PROP-4], pre-filled with the grant/revoke Msg ([VFE-TX-FALLBACK]). The Grant operator form MUST let the user pick the Msg types to grant, all delegable types preselected, rather than granting the full delegable set silently.
- [VFE-PAGE-CORP-6] **Agent Authorizations**: the Corporation's active `VSOperatorAuthorization` entries ([`IDX-DE-QRY-2`](../verana-indexer/spec.md#idx-de-qry-2-list-vs-operator-authorizations) with `corporation_id=<acting>&only_active=true`), one row per `vs_operator` account with its `ParticipantAuthorizationRecord[]` — per record: the target Participant (linked to its detail card, with schema/ecosystem context via its `ecosystem_id`), `msg_types[]`, spend and fee limits, `with_feegrant`, and `expiration`. This view is **read-only**: per VPR, VSOA records are created, updated, and revoked exclusively by the Participant-module flows (`StartParticipantOP` / `SelfCreateParticipant` / `CreateRootParticipant` declare them; revocation/slash/cancel remove them), so the frontend MUST NOT offer direct grant/revoke actions here. The agent-centric view of the same data is the **Agents** page ([VFE-PAGE-AGENTS]); the two surfaces MUST cross-link.
...
- [VFE-PAGE-CORP-8] **Governance version targeting.** A new document (`MOD-GF-MSG-1`) MUST be addressable either to a version already above the active one or to a new version numbered one above the highest known version, and to no other. Activation (`MOD-GF-MSG-2`) MUST be offered for the active version plus one only, so versions activate one at a time and never skip. Each document carries its own language ([VFE-GEN-I18N-4]); the section states the active version and marks every other version as draft or superseded.
...
- [VFE-PAGE-PT-3] The **Participant detail card** (from [`IDX-PP-QRY-1`](../verana-indexer/spec.md#idx-pp-qry-1-get-participant)) MUST present: identity (DID with copy/visualizer links, owning `corporation_id`, id, `ecosystem_id`, deposit, effective window, counters); lifecycle timestamps; onboarding process (op_state badge, `op_exp`, escrowed fees/deposit, `op_summary_digest`); business fees (`validation_fees`, `issuance_fees`, `verification_fees`, shown in the schema's pricing asset with the [VFE-TX-COSTS-2] support gate applied); slashing (slashed/repaid amounts and dates); and the activity timeline ([`IDX-PP-QRY-3`](../verana-indexer/spec.md#idx-pp-qry-3-get-participant-history)). When the validator DID of the entry declares an `EcosystemOnboardingService` service entry ([VFE-PAGE-PENDING-4]), the card MUST show the portal link `{serviceEndpoint}/cases/{participant_id}`: **Review in portal** for the validator Corporation, **Follow in the validator's portal** for the Corporation that owns the entry.
...
- [VFE-PAGE-PT-5] **Join affordance.** On a role-directory node the schema's onboarding mode for that role is a badge of its own and the join control keeps the verb, so the mode never replaces it; the control's accessible name MUST carry both. A join the frontend cannot offer, for instance under the pricing gate of [VFE-TX-COSTS-2], MUST leave the control present but disabled, carrying the reason in its tooltip and in text reachable by assistive technology, never silently inert.

### [VFE-PAGE-AGENTS] Agents

*Scope: the VS Agents operating Verifiable Services on behalf of the acting Corporation.*

- [VFE-PAGE-AGENTS-1] **Derivation.** An **agent** is identified by a DID and is derived, not queried directly: the agent set is the set of **unique `Participant.did` values** across the acting Corporation's `Participant` entries ([`IDX-PP-QRY-2`](../verana-indexer/spec.md#idx-pp-qry-2-list-participants) with `corporation_id=<acting>`), limited by default to entries with `participant_state = ACTIVE` (see the state toggle of [VFE-PAGE-AGENTS-7]). Per the VPR [DID ownership invariant](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#did-ownership-invariant), every such DID is owned by the acting Corporation. Discovery MUST NOT be based on `VSOperatorAuthorization` entries: a `Participant` entry can be created for a DID without any VSOA record (manual mode — the corporation signs and pays its own Participant-related transactions directly), and such DIDs still represent valid agents that MUST be shown. VSOA data is joined per Participant only for the delegation details of [VFE-PAGE-AGENTS-4]. An agent appears with its first `Participant` entry and disappears when the Corporation has no `Participant` entry left referencing its DID within the selected state scope ([VFE-PAGE-AGENTS-7]).
- [VFE-PAGE-AGENTS-1a] **Pinned Corporation and Ecosystem DIDs.** The list MUST additionally always include, pinned at the top and regardless of the filters of [VFE-PAGE-AGENTS-7]: **first** the acting Corporation's own `did` (from its Corporation entry), **then** the `did` of every Ecosystem it controls ([`IDX-ES-QRY-2`](../verana-indexer/spec.md#idx-es-qry-2-list-ecosystems) with `corporation_id=<acting>`, deduplicated — several Ecosystems MAY share one DID), **then** the other agents of [VFE-PAGE-AGENTS-1]. Pinned entries MUST appear even when not verifiable, carrying their trust-state icon per [VFE-TRUST-STATE] and a **Corporation** or **Ecosystem** label. When a pinned DID is also derived from `Participant` entries, a single card is shown in its pinned position, carrying both the label and its accreditations.
- [VFE-PAGE-AGENTS-2] **Card identity.** Each agent card MUST render the DID as a Verifiable Service, exactly like other VS cards: trust-state icon ([VFE-TRUST-STATE]) and service name, logo, description, and controller (organization or persona, with country flag) per [VFE-TRUST-CLAIMS] — from one `POST /v4/verifiable-trust/resolve` per agent DID selecting `ecsCredentials`, `participations`, `services`, and `presentations`.
- [VFE-PAGE-AGENTS-3] **Credentials presented.** The card MUST list the DID's presented credentials: its ECS credentials and the non-ECS VTCs from `presentations[].vtcCredentials[]`, each linkable to its Verifiable Presentation URL and showing its Credential Schema and governing Ecosystem.
- [VFE-PAGE-AGENTS-4] **Accreditations.** The card MUST list one row per Participant of the DID (from the resolve `participations`, with the request's `participations.states` selector set to the state scope of [VFE-PAGE-AGENTS-7]): role badge, schema, ecosystem, and state badge per [VFE-TRUST-BADGE]. Delegation data is joined from the acting Corporation's `VSOperatorAuthorization` entries ([`IDX-DE-QRY-2`](../verana-indexer/spec.md#idx-de-qry-2-list-vs-operator-authorizations) with `corporation_id=<acting>`, matched by `participant_id`; one corporation-scoped call is complete, since per VPR [`MOD-DE-MSG-5-2`](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-de-msg-5-2-grant-vs-operator-authorization-basic-checks) a `vs_operator` account belongs to at most one Corporation). A row whose Participant carries a `ParticipantAuthorizationRecord` MUST be expandable — **collapsed by default** — revealing its delegation detail: `vs_operator` account, `msg_types[]`, spend and fee limits, `with_feegrant`, `expiration`, and a link to the Participant detail card ([VFE-PAGE-PT-3]). Rows without a record (manual-mode Participants) render without an expander. One DID MAY legitimately be served by several `vs_operator` accounts across its Participants; the card simply reflects each row's account. No expiration indicators are shown on this page in this revision.
- [VFE-PAGE-AGENTS-5] **Detail.** The card's detail view MUST list the DID Document's declared service endpoints (resolve `services[]` — DIDComm, MCP, A2A, `VsAgentAdminAPI`, …) and, when a `VsAgentAdminAPI` entry is present, MAY deep-link to the agent's administration interface.
- [VFE-PAGE-AGENTS-6] **Freshness and cross-links.** Delegation and Participant events for the acting Corporation MUST refresh the agent set and accreditation rows ([VFE-DATA-WS-3]); a `trust`-relevant event for an agent DID invalidates its resolve cache ([VFE-DATA-RESOLVE-2]). The raw authorization view of the same data is the Corporation page's **Agent Authorizations** section ([VFE-PAGE-CORP-6]); the two surfaces MUST cross-link.
- [VFE-PAGE-AGENTS-7] **Filters.** The page MUST offer two toggles, both **off by default**:
  - **"Include inactive participant entries"** — off: only `participant_state = ACTIVE` entries are considered, both for deriving the agent list ([VFE-PAGE-AGENTS-1]) and for the accreditation rows of every card ([VFE-PAGE-AGENTS-4]); on: entries of **every** state are included (the state badges make the difference visible).
  - **"Show unverifiable agents"** — off: agents whose DID does not currently resolve as **trusted** ([VFE-TRUST-STATE]) are hidden from the list; on: they are shown too, carrying their untrusted / unresolved trust-state icon.

  Neither toggle affects the pinned Corporation and Ecosystem DIDs of [VFE-PAGE-AGENTS-1a], which are always shown.
...
- [VFE-PAGE-DISCOVER-2] The **Join wizard** MUST guide: (1) review ecosystem (identity, EGF through the viewer of [VFE-GEN-SRV-3], trust state); (2) pick schema; (3) pick role (ISSUER / VERIFIER / ISSUER_GRANTOR / VERIFIER_GRANTOR / HOLDER); (4) apply the pricing gate of [VFE-TX-COSTS-2] — for a schema not priced in the native Coin, the wizard MUST stop here with the unsupported-pricing notice — then resolve the path from the schema's onboarding mode for that role, per the VPR mode tables: `OPEN` (issuer/verifier) → **Self-create** (`MsgSelfCreateParticipant`) with DID, effective window, optional fees; `GRANTOR_ONBOARDING_PROCESS` / `ECOSYSTEM_ONBOARDING_PROCESS` (and HOLDER under `ISSUER_ONBOARDING_PROCESS`) → **validator selection**: candidate validators of the required role from `IDX-PP-QRY-2` (`schema_id`, `role`, `participant_state=ACTIVE`, `trust_data=summary`), shown as trust cards with their `validation_fees`; then **Start OP** (`MsgStartParticipantOP`) with the cost preview of [VFE-TX-COSTS-3]; (5) after broadcast, show the created Participant (op_state PENDING) and state that validation continues between the Corporations' VS Agents over DIDComm ([vt-flow](../vt-flow-protocol/spec.md)) — the frontend tracks progress via Participant events. When a candidate validator's DID declares an `EcosystemOnboardingService` service entry ([VFE-OBS-1]), its trust card MUST surface the validator's offer from the service descriptor per [VFE-OBS-3], the wizard MUST record the governance framework acceptance per [VFE-OBS-4] before Start OP, and the case then continues in the frontend ([VFE-OBS-5]).
...
- [VFE-PAGE-PENDING-2] The navigation badge of the Pending Tasks entry carries this count only; proposals awaiting the vote are badged on the Corporation entry ([VFE-CORP-PROP-2]). Both refresh in real time per [VFE-DATA-WS-3].
- [VFE-PAGE-PENDING-3] Where applicant evidence lives in the validator's Ecosystem Onboarding Service, review happens in the frontend through the service's API ([VFE-OBS-7]). Where it happens elsewhere (agent-side, third-party tooling), the frontend MUST NOT pretend to hold it: the task view surfaces the on-chain facts and the on-chain actions only.
- [VFE-PAGE-PENDING-4] **Onboarding case access.** For each pending task, the frontend MUST resolve the DID of the task's validator Participant (the entry referenced by `validator_participant_id`; one [`IDX-VT-QRY-1`](../verana-indexer/spec.md#idx-vt-qry-1-resolve) call per distinct validator DID selecting `services`, cached per [VFE-DATA-RESOLVE-2]) and, when its DID Document declares a service entry of type `EcosystemOnboardingService` ([VFE-OBS-1]), MUST open the task as the onboarding case review of [VFE-OBS-7] instead of the bare Participant card. The on-chain actions of [VFE-PAGE-PT-4] stay available in both views.

### [VFE-PAGE-SET] Settings

- [VFE-PAGE-SET-1] MUST offer locale selection ([VFE-GEN-I18N]) and theme; MUST display the connected network (chain id, endpoints). Settings persist per [VFE-PAGE-SET-2] and work in guest mode.
- [VFE-PAGE-SET-2] **Persistence.** Locale and theme MUST be persisted client-side under the single key `verana.preferences`, holding the selected locale (a BCP 47 tag, or none when the user never chose one) and the theme choice. Preferences are account-independent: they MUST NOT expire with the session of [VFE-WALLET-4] and MUST survive wallet connect, disconnect and account switch. Absent, unreadable or malformed storage MUST fall back field by field to no stored locale (resolution then starts from the browser, [VFE-GEN-I18N-2]) and to the `system` theme; a storage read or write that throws MUST NOT break the page.
- [VFE-PAGE-SET-3] **Theme.** The choices are `light`, `dark` and `system`; `system` is the default and follows the operating system preference. The header toggle of [VFE-GEN-LAYOUT-2] is a two-state control over the resolved theme, so using it always stores an explicit `light` or `dark`; returning to `system` is possible from Settings only.
- [VFE-PAGE-SET-4] **Locale change.** A locale change MUST take effect without a reload: the document language attribute MUST be set to the resolved locale, the document direction to `rtl` when that locale's maximized script is a right-to-left script and to `ltr` otherwise ([VFE-GEN-I18N-3]), and already-rendered copy MUST be re-translated. The resolution of [VFE-GEN-I18N-2] walks the stored selection, then the browser's preferred locales in order, matching each candidate on the exact tag first and on its primary language subtag second. The locales offered MUST be exactly those the build ships a dictionary for.
- [VFE-PAGE-SET-5] **Network card.** MUST show the deployment's chain id, chain RPC endpoint, chain REST endpoint and indexer base URL from [VFE-GEN-ENV]. A value the deployment did not configure MUST be omitted rather than rendered empty. The card is read-only: no endpoint is editable from the UI.
...
- [VFE-OBS-2] **Sessions.** One session per service origin, through the ADR-036 exchange of [OBS-AUTH-PROTO](../onboarding-service/spec.md#obs-auth-proto-account-challengeresponse) (signed payload `obs-auth:<OBS_PUBLIC_URL>:<nonce>`). The signature prompt MUST be preceded by an explanation naming the validator and the service origin. Tokens are bearer values kept in memory, sent only to the origin that issued them, with the acting Corporation in `X-Acting-Corporation`; they are never cookies and never persisted.
- [VFE-OBS-3] **Offer.** Where a service exists, the frontend renders the validator's offer from the descriptor: offered roles with their fee fields and validity period, prerequisites (with the links of the descriptor for missing ones), the steps of the round, the instructions, and the cost data of the pre-flight (`POST /v1/cases/preflight`), shown before the wallet opens. The pricing gate of [VFE-TX-COSTS-2] applies unchanged.
- [VFE-OBS-4] **Governance framework acceptance.** The EGF step renders through [VFE-GEN-SRV-3]. When the chosen validator has a service, the frontend MUST post the acceptance (`ecosystemId`, `gfvId`, `language`, `url`, `digest_sri`) to it ([OBS-APP-GF](../onboarding-service/spec.md#obs-app-gf-governance-framework-acceptance)) after the user's explicit action and before opening the wallet for `StartParticipantOP` or `RenewParticipantOP`; a refused post blocks the transaction with the service's reason. Without a service, the acceptance remains a local gate as before.
- [VFE-OBS-5] **Applicant case view.** The route `/participants/{participant_id}/onboarding` (the default case URL a service sends over DIDComm) renders the case from the service API: derived status with the chain badges of the entry, the service's `hints[]` as explanations next to the matching actions, the steps and their states, the evidence forms (claims generated from the schema's `json_schema` per the claims mode, documents, fields), submission, messages, and the round history. Renewal, cancellation and the lifecycle actions are the chain actions of [VFE-PAGE-PT-4], offered as the indexer allows; a reason collected with such an action is posted to the service as a note with the transaction report.
- [VFE-OBS-6] **External steps.** For a step of type `external` the frontend calls the launch endpoint ([OBS-STEP-EXT](../onboarding-service/spec.md#obs-step-ext-external-steps)) with a `returnUrl` of its own origin, tells the user they are leaving for the validator's site (naming it), and navigates to the launch URL — which MUST be under the service origin, or the handoff is refused. On return the frontend re-reads the case; query parameters of the return redirect are never trusted.
- [VFE-OBS-7] **Validator review.** Pending Tasks merges the chain's pending entries with the service's case statuses and counts. The review screen renders from the service API: submission, documents (downloaded through the service), step results, credential terms, fee terms, the acceptance record, the round history — and the applicant's Proof of Trust from [`IDX-VT-QRY-1`](../verana-indexer/spec.md#idx-vt-qry-1-resolve) as everywhere else ([VFE-TRUST]). The decision actions (return, refuse, reopen, reject, accept, retry, claims completion) follow the service's `availableActions[]` ([OBS-CASE-ACT](../onboarding-service/spec.md#obs-case-act-available-actions)); the accept form carries the final claims, the fee terms and discounts, `effective_until`, and the credential terms and validity period. When the accept answer names the `OPERATOR` submission path, the frontend leads the operator to sign `SetParticipantOPtoValidated` per [VFE-TX-SIGN-1], prefilled from the recorded decision, and reports the transaction.
- [VFE-OBS-8] **Transaction reporting.** After broadcasting any VPR transaction for a case (or a start), the frontend SHOULD report the hash to the service and update it with the outcome ([OBS-INT-TX](../onboarding-service/spec.md#obs-int-tx-transaction-reports-and-confirmation)); reporting failures never block the transaction flow.
- [VFE-OBS-9] **Trust boundary.** Everything a service returns is data: text and Markdown rendered as such, never as HTML ([VFE-SEC-2]); documents downloaded with a content-disposition that prevents inline execution; no service response may trigger a wallet action outside the frontend's own transaction flows; the only external navigation a service can cause is the external-step handoff of [VFE-OBS-6].
- [VFE-OBS-10] **Aggregation.** "My cases" views aggregate the cases of the acting Corporation across the services of its entries' validators, resolving each per [VFE-OBS-1] and holding one session per origin.
- [VFE-OBS-11] **Portal mode.** A frontend deployment MAY be pinned to one service (`NEXT_PUBLIC_OBS_API_URL`): the deployment then fixes the ecosystem, schema and validator context, applies the descriptor's `branding`, and lands on the offer; everything else in this specification applies unchanged.
- [VFE-OBS-12] **Versioning.** The frontend MUST check the descriptor's `apiVersion` ([OBS-API-VER](../onboarding-service/spec.md#obs-api-ver-versioning)) and refuse a service whose major version it does not implement, telling the user which side needs the upgrade.
...
- [Ecosystem Onboarding Service v4 Specification](../onboarding-service/spec.md)
