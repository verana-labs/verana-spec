...
| Concern | Who | How |
| --- | --- | --- |
| Start, renew, cancel an Onboarding Process; withdraw the entry; repay a slashed deposit | applicant operator | wallet-signed `StartParticipantOP` / `RenewParticipantOP` / `CancelParticipantOPLastRequest` / `RevokeParticipant` / `RepayParticipantSlashedTrustDeposit` under an `OperatorAuthorization` of the applicant Corporation ([OBS-LIFE-APP]) |
...
| Decide, and validate on chain when the agent cannot | validator operator | decision recorded through the portal; wallet-signed `SetParticipantOPtoValidated` under an `OperatorAuthorization` of the validator Corporation when `validateFlow` answers `submission` = `OPERATOR`, or as the fallback after an agent failure ([OBS-VAL-TXFAIL]); after validation, `SetParticipantEffectiveUntil`, `RevokeParticipant` and `SlashParticipantTrustDeposit` on the applicant's entry ([OBS-LIFE-VAL]) |
...
sequenceDiagram
    ...
    alt more information needed
        ...
        P->>VA: validateFlow (fees, effective_until, credential terms): claims checked, submission path
...
| `OBS_CREDENTIAL_FORMATS` | OPTIONAL | Comma-separated list of the credential formats the validator offers, among `JSON_LD` (default, and the only value accepted in this revision). Only meaningful when the applicant role is `HOLDER`. With several values the applicant chooses one at start ([OBS-APP-START-5]); the chosen value is passed to `validateFlow` as `credentialFormat`. |
| `OBS_LINKED_VP_POLICY` | OPTIONAL | Whether the issued credential is to be published by the applicant as a Linked Verifiable Presentation of its DID Document: `YES`, `NO`, `APPLICANT_DEFINED_DEFAULT_YES` (default), `APPLICANT_DEFINED_DEFAULT_NO`. Only meaningful when the applicant role is `HOLDER`. `APPLICANT_DEFINED_*` lets the applicant choose at start, preselected with the default ([OBS-APP-START-5]); the resulting value is passed to `validateFlow` as `presentAsLinkedVp` and conveyed to the applicant agent by the validator agent ([`credential-terms`](../vt-flow-protocol/spec.md#credential-terms)). Constrained for ECS schemas by [OBS-BOOT-6]. This variable is the policy source of this revision; a `CredentialSchema` field is planned as the authoritative source ([vpr-spec #171](https://github.com/verana-labs/verifiable-trust-vpr-spec/issues/171)), after which the variable is dropped. || `OBS_REQUIREMENTS_FILE` | OPTIONAL | Path to the requirements file of [OBS-CFG-REQ]: the documents and additional fields the applicant must provide, and per-schema link overrides. When unset, or when the file declares no document and no field, the evidence step of the applicant journey is omitted. |
...
6. **Determine the credential terms.** [OBS-BOOT-6] When the applicant role is `HOLDER`: `OBS_CREDENTIAL_FORMATS` MUST contain `JSON_LD` only; for the ECS Service, Organization and Persona schemas `OBS_LINKED_VP_POLICY` MUST be `YES` (VS-REQ-2 to VS-REQ-4 of the Verifiable Trust specification); for the ECS Badge and UserAgent schemas, whose credentials are AnonCreds and MUST NOT be declared in a DID Document, no credential can be issued through a flow in this revision and the backend MUST refuse to start. The effective format list and policy are served in `/v1/config`.
7. **Reach the validator agent.** Resolve the Admin API origin ([OBS-CFG-ENV-AGENT]), authenticate if required, call [`getAgentInfo`](../vs-agent/spec.md#vsa-adm-ag-info-getagentinfo). [OBS-BOOT-5] `getAgentInfo.did` MUST equal the validator DID.
...
The portal leads to eight VPR delegable messages ([OBS-INT-MSG-1]): `StartParticipantOP`, `RenewParticipantOP`, `CancelParticipantOPLastRequest`, `RevokeParticipant` and `RepayParticipantSlashedTrustDeposit` on the applicant side, `SetParticipantOPtoValidated`, `SetParticipantEffectiveUntil`, `RevokeParticipant` and `SlashParticipantTrustDeposit` on the validator side ([OBS-LIFE]).
...
- [OBS-CASE-3] A case record holds: `participantId`, `applicantCorporationId`, `did`, `role`, `schemaId`, `ecosystemId`, `createdAt`, `lastEventAt`, the derived `status` ([OBS-CASE-STATUS]), and its rounds. A round holds: `kind` (`initial` | `renewal`), `openedAt`, the governance framework acceptance of the round ([OBS-APP-GF]), the submission state and content (proposed claims, documents, fields), the credential terms (`credentialFormat`, `presentAsLinkedVp`, [OBS-APP-START-5]), the validator messages, the decision (final claims, fee terms, `effective_until`, `op_summary_digest`, credential validity period, decided by, decided at), the lifecycle actions ([OBS-LIFE-3]), and the transaction records of the round.
...
- [OBS-CASE-STATUS-3] `COMPLETED`, `CANCELLED`, `REJECTED`, `REVOKED`, `SLASHED`, `REPAID`, `CREDENTIAL_REVOKED` and `ERROR` are terminal for the round; `VALIDATED` is terminal when the applicant role is not `HOLDER`. `EXPIRED` is terminal on a VPR v4 network, where MOD-PP-MSG-2 requires an active entry and the applicant needs a new Onboarding Process, and is left by a renewal round on a network applying VPR v5 ([verifiable-trust-vpr-spec#172](https://github.com/verana-labs/verifiable-trust-vpr-spec/pull/172)); the portal tells them apart with [OBS-LIFE-EXP-2]. `REFUSED` is not terminal: the validator MAY reopen or reject the round, and the applicant MAY cancel the Onboarding Process ([OBS-APP-CANCEL]). `VALIDATION_FAILED` and `ISSUANCE_PENDING_CLAIMS` wait for a validator action ([OBS-VAL-TXFAIL], [OBS-VAL-POST-3]).
...
- [OBS-APP-START-5] Credential terms (`HOLDER` only): the credential format, a selector when `OBS_CREDENTIAL_FORMATS` lists several values and a displayed value otherwise; and the linked VP choice, a tick box preselected per `OBS_LINKED_VP_POLICY` when it is `APPLICANT_DEFINED_*`, a displayed fixed value otherwise, with the explanation that a linked VP makes the credential public in the DID Document of the service. The terms are stored on the round ([OBS-CASE-3]) and are not part of `StartParticipantOP`; the applicant MAY change a choice it owns from the case page until the validator accepts ([OBS-VAL-ACCEPT-2]).
...
- [OBS-APP-STATUS-3] The page MUST offer the applicant's lifecycle actions ([OBS-LIFE-APP]) and show the expiry notices of [OBS-LIFE-EXP].
...
- [OBS-VAL-REVIEW-1] The review page MUST present: the applicant's full Proof of Trust from [`IDX-VT-QRY-1`](../verana-indexer/spec.md#idx-vt-qry-1-resolve) selecting `ecsCredentials`, `presentations`, `participations` and `ecosystems` (trust state, service and controller identity, every presented credential with its schema and governing ecosystem, every accreditation with its role, state and ecosystem); the flow's claims and proofs as exposed by [`listFlows`](../vs-agent/spec.md#vsa-adm-vt-fl-list-listflows); the submitted documents (downloadable per [OBS-AUTHZ-5]) and fields; the requested fee terms; the credential terms of the round ([OBS-APP-START-5]); the governance framework acceptance of the round (version, document, digest, accepted by and when), flagged when that version is no longer the active one; the on-chain facts of the entry; the round history; and the `instructions.validator` text of the requirements file.
...
- [OBS-VAL-POST-1] When the applicant role is `HOLDER`, the case page MUST show the issuance progress from the flow (`CRED_OFFERED`, `COMPLETED`) and, once issued, the credential identifier, its validity period, its `digestJCS` and the on-chain session reference exposed by [`listFlows`](../vs-agent/spec.md#vsa-adm-vt-fl-list-listflows).
- [OBS-VAL-POST-2] After validation, the validator's case page MUST offer the lifecycle actions of [OBS-LIFE-VAL] on the applicant's entry and show the entry's current on-chain state and the expiry notices of [OBS-LIFE-EXP]; it MAY also deep-link to the Verana Frontend's Participant card when `VERANA_FRONTEND_URL` is set.
...
## [OBS-LIFE] Lifecycle Actions

Beyond the onboarding path, the chain lets the applicant Corporation and the validator Corporation act on the applicant's `Participant` entry for its whole life. The portal offers these actions on the case pages, so that they are recorded with the case and the operator is not sent to another tool.

### [OBS-LIFE-COMMON] Common rules

- [OBS-LIFE-1] Every action of this section is enabled per [OBS-CORP-CAPS-1]: the message MUST be listed in the indexer's `corporation_available_actions[]` (applicant side) or `validator_available_actions[]` (validator side) of the entry, and be in the capability set of the acting Corporation. The portal MUST re-read the entry immediately before opening the wallet ([OBS-CORP-CAPS-3]).
- [OBS-LIFE-2] Every action is a wallet-signed transaction of [OBS-INT-MSG-1] with a transaction record ([OBS-TX-WRITE]); its confirmation step MUST describe the effect in plain language ([VFE-TX-SIM-1](../verana-frontend/spec.md#vfe-tx-sim-simulation-and-cost-preview)), as stated per action below.
- [OBS-LIFE-3] Every action is recorded on the current round with the acting operator, the time, the message text when one is collected, and its transaction record; the round history of both case pages shows it. The effect itself is never derived from the frontend: the event-log poll refreshes `P` and re-derives the status ([OBS-INT-IDX-POLL-3], [OBS-TX-WRITE-3]).
- [OBS-LIFE-4] The same messages executed outside the portal (the Verana Frontend, another operator, the Ecosystem controller, a group proposal) are observed the same way; the portal MUST show them in the round history from the event, with the signer and the transaction hash, and without message text.
- [OBS-LIFE-5] Not offered, and left to the Verana Frontend: group proposals, `CreateRootParticipant`, `TriggerResolver` (executed by the agents), and the actions of the Ecosystem controller on entries validated by another Corporation.

### [OBS-LIFE-APP] Applicant actions

- [OBS-LIFE-APP-1] **Withdraw the entry**: `RevokeParticipant` ([MOD-PP-MSG-9](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-9-revoke-participant)) by the owning Corporation, offered while the entry is `ACTIVE` or `FUTURE` (`op_state` `PENDING` or `VALIDATED`), with a reason (REQUIRED) stored on the round. The confirmation MUST state that the entry ceases to be active immediately and permanently; that for a `HOLDER` entry the applicant's agent deletes the credential and removes its linked VP on the event ([VSA-VTI-NOTIF-PP](../vs-agent/spec.md#vsa-vti-notif-pp-participant-notifications)); that a pending process is not refunded by this message (cancel it first, [OBS-APP-CANCEL]); and that the trust deposit is not released.
- [OBS-LIFE-APP-2] **Repay the slashed deposit**: `RepayParticipantSlashedTrustDeposit` ([MOD-PP-MSG-13](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-13-repay-participant-slashed-trust-deposit)), offered while the entry is `SLASHED`, showing the exact owed amount (`slashed_deposit` − `repaid_deposit`) and the Corporation's balance. The confirmation MUST state that repaying does not reactivate the entry, and that it lifts the gate that blocks any new Onboarding Process or renewal of the Corporation in this ecosystem ([OBS-APP-PRE-5]).
- [OBS-LIFE-APP-3] Renewal and cancellation stay as specified in [OBS-INT-MSG-2] and [OBS-APP-CANCEL].

### [OBS-LIFE-VAL] Validator actions

- [OBS-LIFE-VAL-1] **Set effective until**: `SetParticipantEffectiveUntil` ([MOD-PP-MSG-8](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-8-set-participant-effective-until)), offered while the entry is `ACTIVE` or `FUTURE` with `op_state` = `VALIDATED`: a date-time picker showing the current value, bounded by the entry's `op_exp`, and a reason (OPTIONAL). The confirmation MUST state whether the change extends or reduces the validity, and that the VS operator authorization record of the entry follows the new date ([VSA-VTI-NOTIF-PP](../vs-agent/spec.md#vsa-vti-notif-pp-participant-notifications)).
- [OBS-LIFE-VAL-2] **Revoke**: `RevokeParticipant` ([MOD-PP-MSG-9](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-9-revoke-participant)) as validator, offered while the entry is `ACTIVE` or `FUTURE`, with a reason (REQUIRED) stored on the round and shown to the applicant. The confirmation MUST state the effects of [OBS-LIFE-APP-1] and, for a non-`HOLDER` entry, that the flows the entry serves as validator are terminated by the agents ([VSA-VTI-FLOW-OP-REVOKE](../vs-agent/spec.md#vsa-vti-flow-op-revoke-revoke-participant--slash-participant-trust-deposit)).
- [OBS-LIFE-VAL-3] **Slash**: `SlashParticipantTrustDeposit` ([MOD-PP-MSG-12](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-12-slash-participant-trust-deposit)), offered in any state of the entry, including expired and revoked, with an amount bounded by the entry's `deposit` and a reason (REQUIRED). The confirmation MUST carry the irreversibility warning of [VFE-PAGE-PT-4](../verana-frontend/spec.md#vfe-page-pt-participants) and state that the applicant's agent treats a slash like a revocation ([VSA-VTI-NOTIF-PP](../vs-agent/spec.md#vsa-vti-notif-pp-participant-notifications)).
- [OBS-LIFE-VAL-4] Validation stays as specified in [OBS-VAL-ACCEPT].

### [OBS-LIFE-EXP] Expiry

- [OBS-LIFE-EXP-1] While the entry is `ACTIVE` with a non-null `effective_until`, both case pages MUST show the expiry date; within `NEXT_PUBLIC_SHOW_PARTICIPANT_EXPIRE_BEFORE_DAYS` days of it ([VFE-GEN-ENV](../verana-frontend/spec.md#vfe-gen-env-container-environment-variables), default 30) they MUST show an "expires soon" notice and, on the applicant's page, the renew action ([OBS-INT-MSG-2]).
- [OBS-LIFE-EXP-2] Once the case is `EXPIRED` ([OBS-CASE-STATUS]), the applicant's page MUST state that the accreditation, and the credential when one was issued, no longer count, and MUST offer the way forward: renewal ([OBS-INT-MSG-2]) when the indexer's `corporation_available_actions[]` lists `RenewParticipantOP` for the entry (a network applying VPR v5, [verifiable-trust-vpr-spec#172](https://github.com/verana-labs/verifiable-trust-vpr-spec/pull/172)), otherwise a new Onboarding Process ([OBS-APP-START], allowed because the case is terminal).
...
- [OBS-INT-MSG-1] The portal leads to exactly eight VPR messages ([OBS-LIFE] for the lifecycle ones): `StartParticipantOP` ([MOD-PP-MSG-1](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-1-start-participant-op)), `RenewParticipantOP` ([MOD-PP-MSG-2](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-2-renew-participant-op)), `CancelParticipantOPLastRequest` ([MOD-PP-MSG-6](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-6-cancel-participant-op-last-request)), `RevokeParticipant` ([MOD-PP-MSG-9](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-9-revoke-participant)), `RepayParticipantSlashedTrustDeposit` ([MOD-PP-MSG-13](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-13-repay-participant-slashed-trust-deposit)), `SetParticipantOPtoValidated` ([MOD-PP-MSG-3](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-3-set-participant-op-to-validated)), `SetParticipantEffectiveUntil` ([MOD-PP-MSG-8](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-8-set-participant-effective-until)) and `SlashParticipantTrustDeposit` ([MOD-PP-MSG-12](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#mod-pp-msg-12-slash-participant-trust-deposit)). They are built, signed (Amino by default), fee-granted, simulated, previewed and broadcast per [VFE-TX](../verana-frontend/spec.md#vfe-tx-transaction-execution), by the shared signing utilities of the Verana Frontend codebase, with the transaction-record hook of [OBS-TX-WRITE]. `SetParticipantOPtoValidated` is led to only when `validateFlow` answered `submission` = `OPERATOR`, or as the fallback of [OBS-VAL-TXFAIL-2].
...
| Configuration | `/v1/config` | public deployment descriptor: ecosystem, schema, validator, roles, fees, prerequisites, requirements, claims mode, issuance, credential formats and linked VP policy (serves [OBS-APP-HOME]) |
...
| Cases | `/v1/cases` | list and read cases in the applicant or validator scope; pre-flight and eligibility checks; submissions, drafts, documents, decisions and messages of a case; the accept, reject, retry and claims-completion actions of the validator; the lifecycle actions of [OBS-LIFE] |
...
- **Verana Indexer (v5)**: `corporation_available_actions[]` should list `RenewParticipantOP` for `EXPIRED` entries once MOD-PP-MSG-2 accepts them ([verifiable-trust-vpr-spec#172](https://github.com/verana-labs/verifiable-trust-vpr-spec/pull/172)); until then the portal offers a new Onboarding Process ([OBS-LIFE-EXP-2]).
