...
- Records whose validity window has elapsed (`validUntil < now`, `expiresAtTime < now` on the upstream resolve, or any equivalent expiry signal) MUST be hard-deleted on first observation of the expiry; the graph never returns expired records.
...
The only schema state that can change is the **archived flag**, which is surfaced via the controlling Ecosystem's `ecosystems` channel and applied through diff-from-absence: an archived schema disappears from the resolved Ecosystem's `ecosystems[].credentialSchemas[]` (since [[TG-INGEST-2]] requests `credentialSchemas.includeArchived: false`) and is therefore hard-deleted from the graph along with every edge anchored on it, per [[TG-ACT-1]]. If the same `id` later reappears (un-archived), step 1 applies again; the body fetched on re-load is byte-identical to the previously held one by the immutability invariant.
...
| `staleAfter`          | Wall-clock timestamp after which the record SHOULD be re-resolved (typically derived from upstream `expiresAtTime` and from CGF/EGF freshness windows). |
...
| `ecsSchema`        | enum                   | `ecsCredentials[].ecsSchema`            | `ServiceCredential` \| `OrganizationCredential` \| `PersonaCredential` \| `UserAgentCredential`. Retained for type-based query (alongside the stable `credentialSchemaId`) |
