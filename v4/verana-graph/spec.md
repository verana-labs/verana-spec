...
- `Participant` records are persisted with their current `state`. `ACTIVE` entries are always persisted. A `Participant` that leaves `ACTIVE` MUST be **retained** with its updated state for as long as any persisted record references it (an inbound `ISSUED_BY` / `HELD_AS` edge from an `EcsCredential` or `Vtc`, or a `VALIDATED_BY` edge from a persisted `Participant`); when the last such reference disappears it MUST be hard-deleted together with every edge anchored on it — the same reference-counted mechanic as orphan `Vtc` records below. Non-`ACTIVE` entries that nothing references (e.g. `FUTURE` entries observed at bootstrap) are not persisted. Only `ACTIVE` entries participate in faceted-search joins ([[TG-FCT-3]]); traversal returns retained non-`ACTIVE` entries with their `state`.
- Sub-records carrying their own validity window (e.g. `validFrom`/`validUntil` on credentials, AnonCreds VTC `from`/`to`, or any per-record expiry signal) MUST NOT be returned by any query once the window has elapsed (`validUntil < now`, when non-null) — a **query-time gate**, since a passive wall-clock crossing fires no event; the record is hard-deleted at the next natural reconciliation of its DID. Either way the graph never returns record-level-expired data. The upstream resolve response's top-level `expiresAtTime` is **not** a record-level validity window — it is the DID's *trust-evaluation* expiry horizon, handled separately per [[TG-ACT-3]] (retention + default query filter, not deletion).
...
[TG-ACT-3] The upstream resolve response's top-level `expiresAtTime` (required per [`response.schema.json`](../verana-indexer/schemas/v4/vt/response.schema.json), nullable) is the **earliest future boundary at which the DID would cease to be trustable**, computed by the indexer per [[IDX-VT-EVAL-2]](../verana-indexer/spec.md#trust-evaluation-lifecycle) as the minimum over the trust-evaluation inputs (anchoring `Participant` `effective_until`, ECS credential `validUntil`); it is `null` when no such boundary exists. It is **not** a cache TTL: crossing it means the DID has genuinely stopped being trustable, so the query-time gate below is exact, and no follower-side refresh, sweep, or re-resolve mechanism exists or is needed — a DID becomes trustable again only through an upstream re-evaluation event ([[IDX-VT-EVAL-3]](../verana-indexer/spec.md#trust-evaluation-lifecycle): state-affecting message, `TriggerResolver`, or dependency cascade), which reaches the graph as an ordinary change envelope per [[TG-INGEST-4]]. The graph MUST persist `expiresAtTime` (including `null`) on the corresponding `Did` record (see [`Did`](#did) entity table). A DID is **trust-expired** when its persisted `expiresAtTime` is non-null and `< now` at query time; a `null` horizon never trust-expires. Trust-expired DIDs MUST NOT be hard-deleted by any rule of [[TG-ACT-1]]; they MUST be retained for referential integrity, in the same spirit as archived Ecosystems and CredentialSchemas under [[TG-ACT-2]]. Concretely:
...
- The gate is purely query-time (`expiresAtTime` non-null and `< now`) evaluated against the persisted value: when an upstream re-evaluation event refreshes the DID with a future or `null` `expiresAtTime` (delivered as a change envelope and applied per [[TG-INGEST-4]] / [[TG-INGEST-5]]), the trust-expired predicate lifts automatically at the next query. No deletion / undeletion is involved, and the graph itself never initiates a refresh — the trigger is always upstream, per [[IDX-VT-EVAL-3]](../verana-indexer/spec.md#trust-evaluation-lifecycle).
...
Per the indexer specification, the `subscribeChanges` `participations` channel fires on **every** state transition of a `Participant` involving a subscribed DID, including the time-driven ones (`FUTURE → ACTIVE`, `ACTIVE → EXPIRED`). On each notification the graph re-resolves with the request above — which returns **all** states — and reconciles per [[TG-ACT-1]]: `ACTIVE` entries are inserted or updated, non-`ACTIVE` entries update the `state` of a retained record (when referenced) or remove it (when not), and entries absent from the response are hard-deleted per [[TG-PROV-1]].
...
Implementations SHOULD additionally index the *subject claims* of these VTCs: HTTP-GET the VP URL, re-verify the VP signature against the holder's DID Document, extract each credential whose `id` appears in `vtcCredentials[].id`, and persist its body alongside the corresponding `Vtc` record. This body fetch is RECOMMENDED because two discovery surfaces degrade without it — the domain-credential free-text slot of [[TG-FCT-4]] (e.g. *“Verifiable Services presenting a Diploma credential whose `degreeType = MSc`”*) falls back to schema-level text only, and the `attributes` field of the `credentials` snippet group of [[TG-FCT-6b]] is `null`. It is subject to the per-block dedup of [[TG-DEREF-4]], and SHOULD be skipped when the live envelope's `presentations` channel flag is `false` (the VP contents cannot have changed in that case).
...
[TG-PROV-1] On every resolve response received per [[TG-INGEST-3]] / [[TG-INGEST-4]] / [[TG-INGEST-5]], every persisted field whose value differs from the new response MUST be overwritten and `lastObservedAtTime` and `lastObservedAtBlock` updated. Records or edges no longer present in the new response, or whose record-level validity window has elapsed, MUST be hard-deleted per [[TG-ACT-1]]; `Participant` records that leave `ACTIVE` follow [[TG-ACT-1]]'s retain-while-referenced rule instead. Trust-expired DIDs (per [[TG-ACT-3]]) are NOT hard-deleted; they are retained and gated only at query time. The graph does NOT retain history.
...
| `expiresAtTime`   | timestamp (nullable)                | `expiresAtTime`                                                                                                          | Earliest boundary at which this DID would cease to be trustable, per [[IDX-VT-EVAL-2]](../verana-indexer/spec.md#trust-evaluation-lifecycle); `null` when no boundary exists (never trust-expires until an upstream re-evaluation). When non-null and `< now` the DID is **trust-expired** per [[TG-ACT-3]]: the record is retained for traversal, but the DID — and any `Corporation` / `Ecosystem` whose current `did` equals this DID — MUST NOT appear as a direct hit on the trust surface (no opt-in). Always present, possibly `null` (required by the indexer's response schema) |
| `corporationId`   | uint64                              | `corporationId`                                                                                                         | Stable `uint64` id of the DID's **owner Corporation** per the VPR [DID ownership invariant](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#did-ownership-invariant). Never null. Direct edge anchor for `OPERATED_BY: Did → Corporation`. The per-Participant `vsOperator` accounts live on `Participant` records, not here — a `vsOperator` is a Participant-scoped grant, not a DID-scoped one |
| `pattern`         | enum (`A` \| `B`) \| `null`         | derived during ingestion (see [[def: architecture pattern]])                                                            | `A` = self-issued; `B` = delegated — the anchor determination of [[IDX-VT-EVAL-1]](../verana-indexer/spec.md#trust-evaluation-lifecycle) ([VS-REQ-3] / [VS-REQ-4]). `null` when the DID presents no `ServiceCredential`: such DIDs are surfaced by the indexer with `trusted: false` and persisted per [[TG-INGEST-3]] |
| `serviceTypes`    | string[]                            | `services[].type` (DID Document service entries)                                                                        | Search-facing facet (e.g. `did-communication`, `VsAgentAdminAPI`, `MCP`). For the VS-level service type asserted by the `ServiceCredential`, see [`EcsCredential.ServiceCredential.type`](#ecscredential) |
...
| `corporationId`           | uint64          | `ecosystems[].corporationId`                                                                      | edge anchor to `Corporation` by stable `uint64` id (mirrors VPR `Ecosystem.corporation_id`) of the controlling Corporation. Surfaced per entry for self-describing streaming; per the VPR [DID ownership invariant](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#did-ownership-invariant) it equals the envelope-level `corporationId` |
...
| Field              | Type                   | Source                                  | Notes                                                                                                          |
| ---                | ---                    | ---                                     | ---                                                                                                            |
| `id`               | string                 | `ecsCredentials[].id`                   | **Identity**, jointly with `subjectDid` — the record key is the composite `(subjectDid, id)`. `id` is the VC's mandatory `id` attribute (required upstream)              |
...
| `digestJCS`        | string                 | `ecsCredentials[].digestJCS`            | informational — the credential's JCS digest, recomputed and ledger-verified by the indexer per [[IDX-VT-EVAL-1]](../verana-indexer/spec.md#trust-evaluation-lifecycle) |
| `issuedAtTime`     | timestamp              | `ecsCredentials[].issuedAtTime`         | effective issuance time — the `created` timestamp of the credential's ledger `Digest` entry (per [[IDX-VT-EVAL-1]](../verana-indexer/spec.md#trust-evaluation-lifecycle)) |
...
| `vsOperator`               | account \| `null`             | `participations[].vsOperator`                                                                           | Cosmos account address authorised (via VS Operator Authorization granted by the controlling Corporation, when one exists, signed by its `policyAddress`) to act as the VS operator for **this specific Participant entry**. `null` until such a grant has been made (observed live: most Participant entries carry none). Different Participants of the same DID under the same Corporation MAY carry different `vsOperator` accounts. Surfaced per entry because each Participant's VS Operator Authorization grant is a distinct on-chain object |
...
| `state`                    | enum                          | `participations[].state`                                                                                | current lifecycle state. Non-`ACTIVE` records are retained only while referenced (per [[TG-ACT-1]]) and never match faceted-search joins |
...
The primary key is `id` (VPR `Participant.id`). Renewal recycles the same entry, so `id` is stable across normal renewals; if VPR allocates a replacement entry for the same `(corporation, did, schema, role)` tuple (e.g. after a business-rule change), it carries a new `id` and is treated as a brand-new record, while the old one follows [[TG-ACT-1]]'s retain-while-referenced rule. The persisted `didId` is always the resolved DID under which this Participant was surfaced (envelope-derived); the same VPR Participant entry will never produce two records with different `didId`s because `Participant.did` is itself stable per the entry's lifetime.
...
| ---                    | ---                                                      | ---                                                                                                         |
| `OPERATED_BY`          | `Did` → `Corporation`                                    | `Did.corporationId` matches `Corporation.id` (the DID's owner Corporation per the VPR [DID ownership invariant](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#did-ownership-invariant), surfaced by the indexer as the top-level `corporationId` scalar; always present) |
...
| **B2** | Holder recovery   | `did`, `credentialId` (same resolution as B1)      | `{ credential, subjectDid, holderParticipant }`                   | `{ EcsCredential \| Vtc } —HELD_AS→ Participant`; for `EcsCredential` the subject DID is also `EcsCredential.subjectDid`. `holderParticipant` is `null` for a self-issued (Pattern A) credential, which has no HOLDER `Participant` |
...
[TG-QRY-6] **Pagination.** The queries whose output size scales with ecosystem adoption — **A4, A5, A6, A7, C1, C2, D1, D2, E1, E2** — MUST support cursor-based pagination: an optional request `limit` (min 1, max 500, default 100) and an opaque request `cursor`, with a `nextCursor` in the response envelope (`null` on the last page). Cursor semantics follow [[TG-FCT-7]] — opaque to the client, offset-based pagination MUST NOT be used. The shape-fixed queries (A1, A2, A3, B1, B2, C3, E3, F1, G1) return naturally bounded outputs and are not paginated; they MUST ignore `limit` / `cursor` and return `nextCursor: null` when the field is present.

Each paginated query iterates a **deterministic total order over a stable record key**:

| Query | Canonical iteration order |
| --- | --- |
| A4 | `LinkedVerifiablePresentation.id` ascending |
| A5 | credential `id` ascending (the `subjectDid` component of the key is fixed by the input) |
| A6, D1 | `EcsCredential` records first, by `(subjectDid, id)` ascending; then `Vtc` records by `id` ascending |
| A7, C2, D2 | underlying `Participant.id` ascending (for C2/D2 the `role` grouping of the output is preserved within each page) |
| C1 | `CredentialSchema.id` ascending |
| E1 | `did` ascending |
| E2 | `Ecosystem.id` ascending |

Cursors over these orderings are **keyset** cursors ("continue after key K") and therefore remain valid under concurrent additions and removals: VPR ledger ids are allocated monotonically, so a record created mid-iteration sorts after any existing anchor and appears in a later page of the same iteration; a removed record — including the anchor itself — simply stops appearing. Pages reflect the graph's current state at the time each page is served (no snapshot isolation across pages, consistent with [[TG-ACT-1]]'s live-ingestion model). The `INVALID_CURSOR` error of [[TG-ERR-1]](#error-responses) is accordingly reserved for malformed cursors, cursors replayed against a different query or input than the one that minted them, and implementation-versioned cursors after an internal migration — it does not occur for well-behaved clients mid-iteration.
...
The normative JSON Schema for the traversal request is published alongside this document at [`schemas/v4/graph/traverse/request.schema.json`](./schemas/v4/graph/traverse/request.schema.json). It defines the `query` selector (one of `A1`–`A7`, `B1`–`B2`, `C1`–`C3`, `D1`–`D2`, `E1`–`E3`, `F1`, `G1` per [[TG-QRY-3]]), and the per-query `input` object whose shape depends on the selected query (for example `{ did }` for A1–A7, `{ did, credentialId }` for B1–B2, `{ ecosystemId }` for C1–C3, `{ credentialSchemaId }` for D1–D2, `{ corporationId }` for E1–E3, `{ from: { id, type }, to: { id, type } }` for F1, `{ participantId }` for G1). Optional per-query filters (`ecsSchema` on A5/A6, `role` on A7/C2/D2, `credentialSchemaId` on C2) are carried inside `input`. The top-level optional `limit` and opaque `cursor` implement [[TG-QRY-6]] pagination for the queries it names.
...
- `Did.expiresAtTime` is null OR `>= now` — **NOT overridable** ([[TG-ACT-3]]).
...
- `ServiceEndpoint` hits **mirror their owning DID's gates** (the `Did` of the `EXPOSES_SERVICE` edge): hidden when the owning DID is not `trusted` — overridable via the same `includeUntrusted = true` flag — and hidden when it is trust-expired — **NOT overridable** ([[TG-ACT-3]]). The endpoint surface has no gates of its own.

These gates are **source-surface direct-hit gates**: they apply only to the entity surface being queried, gating whether a record may appear *as a hit on its own surface*. The `ServiceEndpoint` surface is the deliberate exception: an endpoint is a projection of its owning DID, not an independent trust subject, so its hits are gated by the owning `Did`'s gates above. They MUST NOT propagate through (a) cross-surface filter joins declared in [[TG-FCT-3]] nor (b) denormalised free-text fields declared in [[TG-FCT-4]]. A filter that references an archived `Ecosystem` / `CredentialSchema` id or a trust-expired DID MUST resolve as a normal opaque-id join, and free-text matches against text denormalised from such an entity onto the hit's index document MUST contribute to the hit's score. The hit itself is still subject to its own surface's gates. This is the search-surface mirror of the traversal preservation rules in [[TG-ACT-2]] / [[TG-ACT-3]] — an entity hidden as a direct hit remains visible *by reference* through everything pointing to it.

Worked example: a `Did`-surface query for *"plumber issuers"* (free-text *"plumber"* + `Participant.role = ISSUER` + optionally `Participant.credentialSchemaId = N`) MUST surface every `Did` that has an `ACTIVE` ISSUER `Participant` against a schema whose denormalised text matches *"plumber"* (per [[TG-FCT-4]]), even if that schema is archived — because faceted-search joins match only `ACTIVE` Participants ([[TG-ACT-1]]) and Participants against an archived schema remain `ACTIVE` until revoked, expired, or slashed. The schema itself remains hidden on the `CredentialSchema` surface (subject to its own `archived` gate); the issuer `Did`s of that archived schema are surfaced on the `Did` surface (subject to the `Did` surface's `trusted` / `expiresAtTime` gates). The same applies symmetrically to VERIFIER, GRANTOR, and ECOSYSTEM roles, and to filters referencing an archived `Ecosystem` id or a trust-expired DID.

[TG-FCT-3] **Required filter fields per surface.** Implementations MUST honour the operators listed for each field on the corresponding surface. Implementations MAY expose additional filters; they MUST NOT omit any. Filters on `Participant.*` fields join **only `state = ACTIVE`** entries per [[TG-ACT-1]] — retained non-`ACTIVE` Participants never influence search results.
...
| `Did.operatorName`                                   | eq, in, prefix         | derived facet, the same value as `operator.name` of [[TG-FCT-6b]]: the operative `OrganizationCredential.name` or `PersonaCredential.name` (Pattern A: self; Pattern B: issuer). One field rather than one per credential type, so a client filtering by operator name does not have to branch on `operatorKind` first |
...
The `facets` object MUST contain an aggregation for every filter field that the request's `filters` references **with an `eq` or `in` operator** — bare-scalar and array values included, since they are sugar for `eq` / `in`; a field referenced only with another operator (e.g. `prefix`, `range`) requires no aggregation — so a filtered result always carries the drill-down counts for its own filter dimensions. It SHOULD additionally carry an implementation-defined default set of aggregations useful to the surface (e.g. `Did.operatorKind`, `EcsCredential.ServiceCredential.type`, `OrganizationCredential.countryCode` on the `Did` surface). Aggregations over near-unique-value fields (`OrganizationCredential.lei`, `OrganizationCredential.registryId`, `Did.operatorName`) are never required beyond that referenced-filter rule. Facet keys are the filter field names exactly as declared for the queried surface in [[TG-FCT-3]] — identical to the keys the client uses in `filters` (the dotted form on the `Did` surface, the bare form on the other surfaces); implementations MAY truncate each aggregation to a top-N of values. The minimum content of each `hit.snippet` is fixed by [[TG-FCT-6a]]; each surface's snippet is additionally composed of the selectable field groups of its [[TG-FCT-6b]] catalogue, projected per the request's `snippet` selector ([[TG-FCT-6c]]).
...
For the **`Did` surface**, the core additionally carries the flags below on every hit, regardless of the `snippet` projection of [[TG-FCT-6c]]:
...
| Field | Source | Null when |
| --- | --- | --- |
| `pattern` | `Did.pattern` ([[def: architecture pattern]], `A` \| `B`) | never (the group is `null` when there is no `ServiceCredential`) |
| `name` | `ServiceCredential.name` | never |
| `type` | `ServiceCredential.type` | never |
| `description` | `ServiceCredential.description` — implementations MAY truncate for card use; a truncation MUST end at a character boundary and be flagged with a trailing `…` | never |
| `logoUri` | `ServiceCredential.logoUri` | never |
| `logoDigestSri` | `ServiceCredential.logoDigestSri` | never |

**`operator`** (object \| `null`) — the operative ORG-or-PERSONA identity the VS's trust chain rests on (Pattern A: self; Pattern B: issuer), per the derivation of [[TG-FCT-3]]. `null` when the trust chain is incomplete.

| Field | Source | Null when |
| --- | --- | --- |
| `kind` | derived facet (`Organization` \| `Persona`, per [[TG-FCT-3]]) | never |
| `name` | operative `OrganizationCredential.name` or `PersonaCredential.name` | never |
| `logoUri` | operative `OrganizationCredential.logoUri` or `PersonaCredential.avatarUri` | Persona without avatar |
| `logoDigestSri` | paired `OrganizationCredential.logoDigestSri` or `PersonaCredential.avatarDigestSri` | same as `logoUri` |
| `countryCode` | operative `OrganizationCredential.countryCode` or `PersonaCredential.controllerCountryCode` | never |
| `registryId` | operative `OrganizationCredential.registryId` | the operator is a Persona |
| `address` | operative `OrganizationCredential.address` | the operator is a Persona |

**`corporation`** (object, never `null`) — the owner Corporation per the VPR [DID ownership invariant](https://verana-labs.github.io/verifiable-trust-vpr-spec/versions/v4/#did-ownership-invariant), through the `Did.corporationId` anchor.

| Field | Source | Null when |
| --- | --- | --- |
| `id` | `Did.corporationId` | never |
| `deposit` | owner `Corporation.deposit` | owner `Corporation` record not yet materialised (bootstrap transient) |
| `slashedEvents` | owner `Corporation.slashedEvents` (`0` when never slashed) | owner `Corporation` record not yet materialised (bootstrap transient) |
| `lastSlashedAtTime` | owner `Corporation.lastSlashedAtTime` | never slashed, or owner `Corporation` record not yet materialised |
| `slashedValue` | owner `Corporation.slashedValue` | never slashed, or owner `Corporation` record not yet materialised |

**`endpoints`** (array) — the DID's `ServiceEndpoint` records (the non-`LinkedVerifiablePresentation` `services[]` entries of the DID Document), each projected as `{ id, type, serviceEndpoint }` with `serviceEndpoint` preserved verbatim per [DID-CORE]. `[]` when the DID Document declares no such entry.

**`ecosystems`** (array) — the `Ecosystem` entries whose current `did` is this DID (the DID is an ecosystem controller). `[]` when none; non-empty iff the core `isEcosystem` flag is `true`. Archived entries appear with their `archived` flag — these are references from the hit, not direct `Ecosystem`-surface hits, so the [[TG-FCT-2]] gates do not apply to them. Each entry:

| Field | Source | Null when |
| --- | --- | --- |
| `id` | `Ecosystem.id` | never |
| `archived` | `Ecosystem.archived` | never |
| `participants` | `Ecosystem.participants` — per-role aggregate counts (map role → uint) | never |
| `schemas` | one entry per member of `Ecosystem.credentialSchemaIds[]`, each `{ id, title, archived, participants }`: `id` and `archived` from the `CredentialSchema` record, `title` from the loaded `CredentialSchema.body` (`null` when the body declares no title), `participants` the per-role counts of `CredentialSchema.participants` | never — `[]` when the Ecosystem owns no schema |

**`participations`** (object) — the DID's **`ACTIVE`** `Participant` entries. Non-`ACTIVE` entries never reach the search surface, per [[TG-ACT-1]]; the full lifecycle view is the [`A7`](#a-did-rooted) traversal. Shape: `{ total, ecosystemCount, byRole, entries }`.

| Field | Source | Null when |
| --- | --- | --- |
| `total` | count of the DID's `ACTIVE` `Participant` entries | never |
| `ecosystemCount` | count of distinct `Participant.ecosystemId` values among them | never |
| `byRole` | map role → count over the same entries | never — `{}` when `total = 0` |
| `entries` | one entry per `ACTIVE` `Participant`, each `{ id, role, credentialSchemaId, schemaTitle, ecosystemId }`: `id` the stable VPR `Participant.id` (the [`G1`](#g-participant-rooted) traversal anchor), `role` per the [`Participant`](#participant) entity table, `schemaTitle` from the loaded `CredentialSchema.body` (`null` when the body declares no title) | never — `[]` when `total = 0` |

**`credentials`** (object) — the **non-ECS** Verifiable Trust Credentials presented by the DID: the `Vtc` records reachable via `Did —REFERENCES_VP→ LinkedVerifiablePresentation —CONTAINS_VTC→ Vtc`. ECS credentials (`ServiceCredential`, `OrganizationCredential`, `PersonaCredential`) MUST NOT appear in this group — they are already projected by the `service` and `operator` groups. A presented VTC is by definition held by the DID, so entries carry no role field. Shape: `{ total, entries }`.
...
[TG-FCT-8] **Composition with traversal.** Faceted-search returns ranked hits carrying the data selected by the [[TG-FCT-6c]] projection — at minimum the [[TG-FCT-6a]] core, at most every group of [[TG-FCT-6b]]; deep enrichment beyond the groups (full governance chain, per-credential issuer recovery, validator chains, lifecycle states of non-`ACTIVE` Participants, etc.) is the job of [Graph-traversal Queries](#graph-traversal-queries) from the hit's id. Implementations MAY expose composite façades that fold both layers into a single request (e.g. "search returning top-N hits plus A1 + A2 inlined per hit"), but the contract — [[TG-FCT-1]] through [[TG-FCT-7]] plus the [[TG-QRY-3]] traversal contracts — remains the unit of conformance.
  ...
  "hits": [
    {
      ...
      "snippet": {
        ...
        "service": {
          ...
          "name":          "Fabrice Agent",
          "type":          "AIAgent",
          "description":   "Personal AI agent of @fabrice.",
          "logoUri":       "https://fabrice.agents.example/logo.png",
          "logoDigestSri": "sha384-…"
        },
        "operator": {
          "kind":          "Persona",
          "name":          "@fabrice",
          "logoUri":       "https://fabrice.agents.example/avatar.png",
          "logoDigestSri": "sha384-…",
          "countryCode":   "CO",
          "registryId":    null,
          "address":       null
          ...
          "entries": [
            {
              ...
              "schemaTitle":        "Service Credential",
              "ecosystemId":        1
            },
            {
              "id":                 977,
              "role":               "HOLDER",
              "credentialSchemaId": 9,
              "schemaTitle":        "AI Agent Certification",
              "ecosystemId":        7
            }
          ]
        },
        "credentials": {
          ...
          "entries": [
            {
              "id":                 "urn:uuid:0e6f2a34-9d1c-4c6e-8f4b-2b7d1a0c5e21",
              "credentialSchemaId": 9,
              "schemaTitle":        "AI Agent Certification",
              "ecosystemId":        7,
              "attributes": {
                "certificationLevel": "L2",
                "capabilities":       ["chat", "task-automation"]
              }
            }
          ]
        }
    ...
    },
    {
      ...
      "snippet": {
        ...
        "service": {
          ...
          "name":          "Fabrice Bot",
          "type":          "AIAgent",
          "description":   "Task automation agent of @fabrice-bot.",
          "logoUri":       "https://fabrice-bot.agents.example/logo.png",
          "logoDigestSri": "sha384-…"
        },
        "operator": {
          "kind":          "Persona",
