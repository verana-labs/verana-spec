# The Utopia Use Case — Playground Specification
...
| Name | Role |
|---|---|
| **Republica of Utopia** | Protagonist: a small democracy going digital. Brand kit: flag, coat of arms, institution imagery (generated, in-repo). |
...
| **National Civil Registry (demo)** | Operates the **Utopia Citizen ID** trust registry and issues the Citizen ID (issuance governed, verification governed). |
...
| **Verana ECS Ecosystem** | The shared identity-card trust registry (exists on testnet). Utopia **joins** it for Business IDs — with the explicit design note that it could have built a custom ecosystem instead, but interop wins: Utopian companies are trusted everywhere, not just in Utopia. |
...
1. `/usecases/utopia` — **Meet the Republica of Utopia**
2. `/usecases/utopia/solution` — **The solution: a verifiable Republic**
3. `/usecases/utopia/journey` — **The Minister's journey**
4. `/usecases/utopia/demos` — **Run the demos**

### Chapter 1 — Meet the Republica of Utopia *(civic article; no DIDs, no protocol)*
...
- **The ecosystems Utopia wants to join** — the **Verana ECS Ecosystem** (HOLDER + ISSUER roles): Business IDs are plain **ECS-Organization** credentials, issued by the National Business Registry once accredited. Design note, stated in the story: *Utopia could run Business IDs in a custom ecosystem, but joining the shared ECS ecosystem means a Utopian company's credential is recognized by every Verana-aware wallet and service worldwide — interop wins.*
- **The ecosystems Utopia wants to build** — two cards (ECOSYSTEM role badges):
  - **Utopia Citizen ID** (National Civil Registry) — issuance governed (only the Civil Registry issues), **verification governed** (relying parties must register — the eIDAS 2 relying-party analog).
  - **Utopia Legal Representation** (National Business Registry) — issuance governed (only the Business Registry issues), verification OPEN (anyone may check who represents a company — that is the register's public function).
...
Amber standing rule (always verify the certified Organization in the Proof-of-Trust first) · wallet chooser (reused component; carries the "any integrated wallet can be customized for Utopia" message) · then:

1. **Get your Utopia Citizen ID** — live offer minted by the Civil Registry agent; green Proof-of-Trust (certified institution, authorized issuer). SD-JWT for OID4VC wallets (the eIDAS-compatible rail), AnonCreds for Hologram. Demo claims: a generated citizen identity (per-scan identifier, e.g. `UT-XXXXXXXX`). Red issuance paths are the six-scenario page's job and are not duplicated here.
...
6. **Search the Directory of Utopia** — Trust Graph teaser (demo-coming chips): *all verified Utopian businesses* (ECS-Org issued by the National Business Registry) · *all services accepting the Utopia Citizen ID* (authorized verifiers) · *who legally represents company X*.

Closing teaser: **Being found in Utopia** — everything the Republic and its companies published is public, resolvable, and indexable; the Trust Graph turns it into a national service directory nobody has to maintain by hand. Ships when discovery is queryable.
...
### Utopia Citizen ID *(Civil Registry ecosystem)*
...
| `personalIdentifier` | string | `UT-` prefixed, unique per citizen (random per scan in the demo) |
| `nationality` | string | `UT` |
...
| `companyRegistryId` | string | Utopia register number |
...
Same rules as the Vesta cast: **one vs-agent (Business Wallet) per participant**, hosts on `*.utopia.playground.testnet.verana.network`, provisioned by `utopia-*` CI/CD workflows cloned from the `vesta-*` pattern (shared `common.sh`, per-org `config.env`, schema JSONs, Admin-API scripts). Status: ✔ exists · ◐ partial · ☐ to build.
...
| Item | Status | Notes |
|---|---|---|
| vs-agent + did:webvh | ☐ | `business-registry.utopia.playground.testnet.verana.network` |
...
| Item | Status | Notes |
|---|---|---|
| vs-agent + did:webvh | ☐ | `civil-registry.utopia.playground.testnet.verana.network` |
| ECS-Organization (Business Registry) + self-issued ECS-Service | ☐ | the dogfooding step, 3.2 |
| **Utopia Citizen ID** trust registry + EGF + schema + root permission | ☐ | issuance ECOSYSTEM, verification ECOSYSTEM |
...
| Item | Status | Notes |
|---|---|---|
| vs-agent + did:webvh + ECS creds (ECS-Org from the Business Registry) | ☐ | `tax-buro.utopia.playground.testnet.verana.network` |
...
| Item | Status | Notes |
|---|---|---|
| vs-agent + did:webvh + ECS creds (ECS-Org from the Business Registry) | ☐ | `meridian-bank.utopia.playground.testnet.verana.network` |
...
| Item | Status | Notes |
|---|---|---|
| vs-agent + did:webvh + ECS creds (ECS-Org from the Business Registry) | ☐ | `quickcash.utopia.playground.testnet.verana.network` — genuinely verifiable |
...
6. **Directory of Utopia**: wire live queries when the Trust Graph ships; consider an interim indexer query (participants of the Citizen ID schema tree = the public relying-party register — itself a nice transparency story).
